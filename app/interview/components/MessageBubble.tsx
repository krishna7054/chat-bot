...
export default function MessageBubble({ msg }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[88%] rounded-3xl border px-4 py-3 text-sm shadow-sm sm:max-w-[80%] sm:px-5 sm:py-4 sm:text-[15px] ${
          isUser
            ? "border-[rgba(143,79,25,0.22)] bg-[linear-gradient(135deg,#c67a3f,#9d5922)] text-white shadow-[0_16px_36px_rgba(143,79,25,0.25)]"
            : "border-white/70 bg-white/80 text-(--foreground) shadow-[0_12px_28px_rgba(88,61,31,0.08)] backdrop-blur"
        }`}
      >
